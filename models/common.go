...
var regTplFuncs = []regTplFunc{
	...
	// Regular expression for matching {{ TrackLink .Subscriber.Attribs.url }} (attribute/variable
	// reference without quotes) and substituting with {{ TrackLink .Subscriber.Attribs.url . }}
	// to add the dot context. This allows tracking links from subscriber/campaign attributes.
	{
		regExp:  regexp.MustCompile(`{{\s*TrackLink\s+(\.[\w.]+)\s*}}`),
		replace: `{{ TrackLink $1 . }}`,
	},
