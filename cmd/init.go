...
func prepareQueries(qMap goyesql.Queries, db *sqlx.DB, ko *koanf.Koanf) *models.Queries {
	...
	// Unique view counts always use the unique-counts query with DISTINCT subscriber_id.
	qMap["get-campaign-unique-view-counts"] = &goyesql.Query{
		Query: fmt.Sprintf(qMap["get-campaign-analytics-unique-counts"].Query, "campaign_views"),
		Tags:  map[string]string{"name": "get-campaign-unique-view-counts"},
	}
